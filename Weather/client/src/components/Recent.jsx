		...
		if (JSON.parse(localStorage.getItem("rc")) !== null) {
			JSON.parse(localStorage.getItem("rc")).map((city, index) => {
				axios
					...
					.then((res) => {
						setTest((test) => [...test, { name: res.data[0].name, state: res.data[0].state, lat: city.lat, lon: city.lon, zip: city.zip, index }]);
					});
			});
		}
		setLoaded(true);
                    ...
                    <ul>
                        {loaded
                            ? test
                                    ...
                                    .map((rc, index) => {
                                        return (
                                            <Link to={`/weather/current/${rc.name}/${rc.lat}/${rc.lon}`} key={index}>
                                                <li key={index}>
                                                    {rc.name}, {rc.state}
                                                </li>
                                            </Link>
